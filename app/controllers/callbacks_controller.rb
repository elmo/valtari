  ...
  def facebook
   ...
   def linkedin
    ...
    if @user.persisted?
      ...
      sign_in(@user)
      if Time.zone.now < (@user.created_at + 10.seconds)
        redirect_to after_sign_up_path_for(@user) and return false
      else
        redirect_to after_sign_in_path_for(@user) and return false
      end
    else
